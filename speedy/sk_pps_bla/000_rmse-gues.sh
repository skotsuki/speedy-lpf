#!/bin/sh
#==================================================#
export ONLYCAT=0 # 0: no  1: only catting mode
#==================================================#
...
SPNDIR=$SPEEDY/DATA/raob/BASELETKF_M000040L0800IADP
CTLDIR=$SPEEDY/DATA/raob/BASELETKF_M000040L0800IADP
...
#SYS="SYS77777777_" # hanshin-hangi (fault; diagonal first --> SK's original) --> returned to be diagonal first 
#SYS="SYS20200528_" # REAMPLE 200 --> 40
...
for HSG    in $HSGTBL ; do
  ...
  H=$( printf %04i ${HSG} )
  BASIC=M${M}L${H}IADP
  if [ $OBS == "reg2" ]; then BASIC_LETKF=M${M}L0500IADP ; fi
  if [ $OBS == "raob" ]; then BASIC_LETKF=M${M}L0800IADP ; fi
