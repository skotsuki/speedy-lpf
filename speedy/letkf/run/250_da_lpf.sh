...
export NODE_list='tsubasa'
export NODE=40                        #Nodes for parallel computing
export MPIRUN=$HOME/local/bin/"mpirun"
LUTDIR='/../../src_LTERPLUT/output/'
#========================================================================== (1) DA OPTIONS
SYS='SYS20200829_RELAXATION_'
MEMTBL='40'
OBSTBL='raob' # raob, reg2, raob
HSGTBL='700 800 900 1000'
#========================================================================== (2) WI OPTIONS
...
RLXTBL='0.55 0.60 0.65 0.70 0.75'   # relaxation parameter alpha (RTPP)
...
if test $NOSAVEENS -eq 1 ; then
  if test `echo $PYMDH | cut -c7-10` != "0100" ; then
      rm -f $TMPDIR/gues/??????/${PYMDH}.grd   $TMPDIR/gues/??????/${PYMDH}_p.grd   \
...
#
# Clean up all ensemble members.
#

rm -f $OUTPUT/anal/000???/$IYYYY$IMM$IDD$IHH*.grd
rm -f $OUTPUT/anal_f/000???/$IYYYY$IMM$IDD$IHH*.grd
rm -f $OUTPUT/gues/000???/$IYYYY$IMM$IDD$IHH*.grd
