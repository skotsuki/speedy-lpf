...
!=======================================================================
SUBROUTINE letkf_core(nobs,nobsl,hdxb,rdiag,rloc,dep,parm_infl,trans,LOGIC_LPFGM,work3,work4)
  IMPLICIT NONE
  ...
  REAL(r_size),INTENT(OUT) :: work3(nbv)
  REAL(r_size),INTENT(OUT) :: work4(nbv,nbv)
  REAL(r_size) :: rho
...
! LOGICAL :: exist
  IF(nobsl == 0) THEN
    ...
    work3(:)   = 0.0d0           ! mean update (LETKF)
    work4(:,:) = SQRT(parm_infl) ! ptb  update (LETKF)
    work5(:,:) = 0.0d0           ! ptb  update (LPFGM)
    RETURN
  ELSE
!-----------------------------------------------------------------------
!  hdxb Rinv
...
!    END DO
!  END DO
  work4(:,:) = trans(:,:) ! <--> A.P 6/12/2018 - Saving W matrix
!-----------------------------------------------------------------------
